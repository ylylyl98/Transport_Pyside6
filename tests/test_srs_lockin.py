...
class SRSLockinPanelTests(unittest.TestCase):
    ...
    def setUp(self):
        self.manager = DeviceManager(Connections())
        self.panel = LockinPanel(self.manager)
        self.panel.exp_settings.set_expanded(True)
    ...
    def test_active_sweep_keeps_settings_visible_but_read_only_until_release(self):
        self.manager.sessions["lockin"] = object()
        self.manager._emit_status(
            "lockin",
            "ok",
            "Stanford Research Systems,SR830,1,1.07",
        )
        sensitivity_index = self.panel.cbo_sensitivity.currentIndex()
        usage_changes = QtTest.QSignalSpy(self.manager.resources_changed)

        claimed, blocked = self.manager.mark_in_use(["daq", "lockin"])

        self.assertTrue(claimed)
        self.assertEqual(blocked, [])
        self.assertTrue(self.panel.cbo_sensitivity.isEnabled())
        self.assertTrue(self.panel.cbo_sensitivity.isReadOnly())
        self.assertTrue(self.panel.sp_phase.isEnabled())
        self.assertTrue(self.panel.sp_phase.isReadOnly())
        self.assertFalse(self.panel.btn_apply.isEnabled())
        self.assertFalse(self.panel.btn_refresh.isEnabled())
        self.assertTrue(all(not button.isEnabled() for button in self.panel.action_buttons.values()))
        self.assertFalse(self.panel.btn_stop_for_settings.isHidden())
        self.assertIn("Locked by active sweep", self.panel.lbl_message.text())
        self.assertIn(self.panel.cbo_sensitivity.currentText(), self.panel.lbl_message.text())
        ...
        self.assertTrue(self.panel.btn_stop_for_settings.isHidden())
        self.assertIn("available again", self.panel.lbl_message.text())
